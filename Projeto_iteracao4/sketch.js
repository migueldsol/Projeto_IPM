...
function buttonSetup(horizontal_gap, vertical_gap, targetSize) {
  ...
  button = new ButtonCat(
    ...
    labels,
    categoryColors,
    [[letterLList, letterMList, letterNList], [letterOList, letterPList, letterRList], [letterSList, letterTList, letterVList], [letterWList, letterYList, letterZList], [number0List, letterAList, letterBList, letterCList], [letterFList, letterGList, letterKList]]
  );
...
function draw() {
  if (draw_targets && attempt < 2) {
    ...
    textFont("Arial", 20);
    textAlign(CENTER);
    text(legendas.getString(trials[current_trial], 0), width / 2 - 20, height - 20);
  } else if (draw_button) {
    // The user is interacting with the 6x3 target grid
    background(color(0, 0, 0)); // sets background to black
...
function mousePressed() {
  ...
  if (draw_targets) {
    ...
    if (varButton[0] != -1) {
      let outerIndex = varButton[1];
      let innerIndex = varButton[0];

      console.log(button.getTargets(), button.getTargets()[outerIndex][innerIndex]);
      wantedTargets = button.getTargets()[outerIndex][innerIndex];
